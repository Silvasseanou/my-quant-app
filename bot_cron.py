...
class DataService:
    ...
    def get_market_wide_pool():
        ...
        try:
            ...
            df = df[mask].dropna(subset=['近6月'])
            df = df.sort_values(by="近6月", ascending=False)
            return [{"code": str(row['基金代码']), "name": row['基金简称']} for _, row in df.head(200).iterrows()]
        except: return []
...
class WaveEngine:
    ...
    def analyze_structure(df_slice):
        """核心波浪诊断逻辑"""
        if len(df_slice) < 60: return {'status': 'Wait', 'score': 0, 'desc': '数据量不足'}
        
        last_nav = df_slice['nav'].iloc[-1]
        ema89 = df_slice['ema_89'].iloc[-1]
        high_20 = df_slice['high_20'].iloc[-2] # 取昨日高点
        ...
        if last_nav < low_20:
            return {'status': 'Sell', 'score': -90, 'desc': '破位：跌破 20 日支撑线'}
            
        # 2. 买入判定
        if last_nav > high_20:
            if ao_curr > 0 and ao_curr > ao_prev:
                return {'status': 'Buy', 'score': 85, 'desc': '突破：20日新高 + 动能确认 (浪3特征)'}
            return {'status': 'Buy', 'score': 70, 'desc': '突破：20日新高 (等待动能放量)'}
            
        return {'status': 'Hold', 'score': 50, 'desc': '震荡运行中'}

# === 3. 自动化任务执行 (The Cron Task) ===

def run_daily_mission():
    bj_now = get_bj_time()
    print(f"⏰ 北京时间 {bj_now.strftime('%Y-%m-%d %H:%M:%S')} 开始执行巡检任务...")
    
    if not SUPABASE_URL or not SUPABASE_KEY:
        print("❌ 错误: 未配置 SUPABASE 环境变量")
        return
    ...
    # --- A. 持仓风险诊断 (SELL 信号) ---
    sell_alerts = []
    print(f"正在诊断 {len(holdings)} 只持仓基金...")
    for h in holdings:
    ...
    # --- B. 全市场 Top200 激进扫描 (BUY 信号) ---
    print("正在扫描全市场 Top 200 机会...")
    buy_opps = []
    market_pool = DataService.get_market_wide_pool()
    
    # 为了防止 GitHub Actions 运行超时，限制扫描前 100 只
    for fund in market_pool[:100]:
    ...
    if buy_opps:
        report_sections.append("🔭 **选股雷达 (强动能 Top 5)**\n" + "\n".join(buy_opps[:5]))
        ...
        payload = {
            ...
            "card": {
                ...
                "elements": [
                    ...
                    {"tag": "hr"},
                    {"tag": "note", "elements": [{"content": f"状态: 持仓 {len(holdings)} 只 | 现金 ¥{capital:,.0f} | 引擎 v37.0", "tag": "plain_text"}]}
                ]
            }
        }
        try:
            requests.post(FEISHU_HOOK, json=payload, timeout=10)
            print("✅ 报告已成功推送到飞书")
